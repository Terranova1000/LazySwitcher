#!/usr/bin/env bash
# ВНИМАНИЕ. Ставит отладочную сборку рядом с релизной, с другим идентификатором.
# НЕ ДЛЯ ПОЛЬЗОВАТЕЛЯ: две одинаковые иконки в строке меню неразличимы, а
# «Проверить обновления» в отладочной всегда отказывает — скачанный релиз не
# может удовлетворить её designated requirement (другой bundle ID), и человек
# видит «подпись не совпадает» про пустое место. Убирайте её за собой.
