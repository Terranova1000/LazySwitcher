...
### 1.7 Перезапуск перехвата: один процесс — один перехват

Пересоздать tap «с нуля» — последнее средство, и у него своя ловушка. Если порт,
источник и сторож хранятся в полях объекта, а перезапуск — это «остановить старый
поток и сразу запустить новый», два потока пишут в одни поля одновременно.
Проигравший гонку сносит чужой перехват или теряет ссылку на свой, который
**остаётся установленным и продолжает доставлять события**. Итог в поле: два
перехвата у процесса, каждое нажатие в буфере дважды (00-DECISIONS, Н60).

Правила, которые из этого следуют:

- всё, что создал поток перехвата, принадлежит этому потоку и больше никому;
- колбэк знает, какой поток его вызвал (refcon — объект потока), и события
  не текущего потока пропускает, ничего не записывая;
- проверять количество перехватов не своей бухгалтерией, а у оконного сервера:
  `CGGetEventTapList` отдаёт все перехваты системы с `tappingProcess`;
- «поток молчит» мерить временем бодрствования: `ProcessInfo.systemUptime`
  (= `mach_absolute_time`) во сне стоит, `Date()` и `mach_continuous_time` идут.
  По стенным часам любой сон выглядит как смерть потока (Н61);
- прежде чем пересоздавать — спросить поток: живой отвечает за миллисекунды.

### 1.8 Переключение раскладки не мгновенно

`TISSelectInputSource` возвращается сразу, а система меняет раскладку позже.
Замерено (00-DECISIONS, Н67): 23–63 мс, если опрашивать состояние самим каждые
5 мс, и 52–277 мс, если ждать уведомления `kTISNotifySelectedKeyboardInputSourceChanged`
— разница целиком в занятости нашего же главного потока.

Всё, что человек набирает в этом окне, система производит **старой** раскладкой.
Поэтому:

- слово, начатое в окне переключения, выходит смешанным, и знать об этом можно
  только пометив каждое нажатие раскладкой (Р26);
- на уведомление нельзя опираться там, где важны десятки миллисекунд: свои
  переключения опрашиваем, чужие ловим быстрым таймером, пока идёт набор.

И обратное: синтетический ввод раскладкой не управляется — текст мы вставляем
через `keyboardSetUnicodeString`, поэтому наши собственные замены переключение
не задевает.
...
**Это значит, что пароль в защищённом поле мы физически не можем прочитать.** Не
«не хотим» — не можем.

⚠️ Но границы этого утверждения уже, чем кажется, и их проверили (`00-DECISIONS.md`,
Н12). Режим включают: родные `NSSecureTextField`, экран блокировки, Терминал с
защищённым вводом, менеджеры паролей. **Не включает его ни один браузер для
`<input type="password">`** — ни Safari, ни Chrome, ни Firefox. Пароль на сайте
Secure Input не защищает.

Поэтому в описании приложения нельзя писать «мы не можем прочитать пароль» без
оговорки. Для веб-полей единственная защита — подроль `AXSecureTextField` (§3.2),
и она работает только там, где браузер отдаёт дерево доступности.
...
4. В меню показать причину — **без имени приложения**. Способ достать PID
   виновника из IORegistry (`IOService:/IOResources/IOConsoleUsers` →
   `CGSSessionSecureInputPID`) на macOS 15.7.7 не работает: ключа там нет даже
   когда режим точно включён (`00-DECISIONS.md`, Н13). Плюс у него была
   известная проблема с неверным определением фонового процесса. Показываем
   нейтральное «ввод защищён системой».
